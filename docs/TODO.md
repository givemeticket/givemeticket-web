...
## 인증/로그인
- [ ] 액세스 토큰 유효성 검증 — 지금은 "저장돼 있는지"만 확인, 만료/위조 여부는 검사 안 함 (`useAuth.ts`). 401 응답이 왔을 때 만료된 토큰을 지우고 로그인 화면으로 보내는 사후 처리는 `axiosClient.ts`에 이미 구현됨 — 이 항목은 그와 별개로, 보호된 화면을 그리기 전에 미리 검증하는 것(예: `/me` API로 사전 확인)
