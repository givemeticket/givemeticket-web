...
- [ ] **스크롤 연동 소개 화면("/", `features/landing`) — 제품 화면 5개 슬롯을
  실제 화면으로 교체** — 구조(다크 티켓월 배경 + 카드 축소 + 캡션/닷 인디케이터,
  `useLandingScrollProgress.ts`)는 구현 완료. 다만 5개 슬롯(행사 개설/나의
  행사/행사 검색/상세·신청/나의 티켓)은 아직 `LandingProductScreens.tsx`의
  자리표시 그래픽임. 사용자가 원하는 최종 형태는 스크롤 위치에 따라 각 화면
  안의 실제 동작(마우스 움직임, 리스트 스크롤 등)까지 스크럽되는 영상
  방식(video의 currentTime을 스크롤 진행률에 매핑) — 화면 녹화 확보 방법
  결정 필요(사용자 직접 녹화 / Playwright 자동화 설치 후 캡처 등). 확보되면
  `LandingProductScreens.tsx`의 자리표시 div만 `<video>`(또는 우선 `<img>`)로
  교체하면 됨(슬롯 구조 자체는 이미 그렇게 준비해둠).
...
**라우팅 구조는 갖춰짐** — 헤더 탭을 "홈(/)/찜한 행사(/mywish)/나의 티켓/나의
행사" 4개로 늘렸고, 로그인 상태로 "/"에 오면 더 이상 `/mytickets`로
리다이렉트되지 않고 헤더+탭이 있는 레이아웃 안에서 "홈" 탭이 보임(비로그인은
지금처럼 안내 화면). 단 "홈"/"찜한 행사" 둘 다 아직 텍스트 한 줄짜리
placeholder라, 아래 실제 기능(찜 API 연동, 홈 개요 화면, 찜 목록 등)은
전부 그대로 남아있음.
